...
const DEFAULT_GOOGLE_MAPS_API_KEY = 'AIzaSyDsx50F-rGzyLaFmn2jpP4nuHjyrEKjf8U';

// Create a singleton loader instance
let loaderInstance: any = null;
let currentApiKey: string | null = null;

// Get API key from settings table or environment variables
const getApiKey = async (): Promise<string> => {
  ...
  try {
    ...
    if (!error && data) {
      try {
        currentApiKey = JSON.parse(data.value);
    ...
    console.warn('Could not get Google Maps API key from settings:', error);
  }
...
export const renderSimpleMap = (
  ...
  loadGoogleMapsApi().then(google => {
    const element = typeof mapElement === 'string'
      ? document.getElementById(mapElement)
      : mapElement;
    if (!element || !(element instanceof HTMLElement)) {
      console.warn('Google Maps element not found or invalid:', mapElement);
      return;
    }
