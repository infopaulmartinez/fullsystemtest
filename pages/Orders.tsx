...
export default function Orders() {
  ...
  const loadOrders = async () => {
    try {
      ...
      if (dateRange.start) {
        adminQuery = adminQuery.gte('order_date', dateRange.start)
      }

      if (dateRange.end) {
        adminQuery = adminQuery.lte('order_date', dateRange.end)
      }

      // Load webshop orders
      let webshopQuery = supabase
        .from('webshop_orders')
        .select('*, pickup_location:pickup_location_id(name, address)')
        .order('created_at', { ascending: false })

      if (statusFilter !== 'all') {
        webshopQuery = webshopQuery.eq('status', statusFilter)
      }

      if (dateRange.start) {
        webshopQuery = webshopQuery.gte('created_at', dateRange.start)
      }

      if (dateRange.end) {
        webshopQuery = webshopQuery.lte('created_at', dateRange.end)
      }

      const [adminData, webshopData] = await Promise.all([
        adminQuery,
        webshopQuery
      ])

      if (adminData.error) throw adminData.error
      if (webshopData.error) throw webshopData.error

      // Combine orders and mark webshop orders
      const combinedOrders: Order[] = [
        ...(adminData.data || []).map(order => ({...order, source: 'admin'})),
        ...(webshopData.data || []).map(order => ({...order, source: 'webshop'}))
      ]
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      const table = order?.source === 'webshop' ? 'webshop_orders' : 'orders'
  ...
  const handleUpdateStatus = async (id: string, status: string, source?: 'admin' | 'webshop') => {
    try {
      const currentOrder = orders.find(o => o.id === id)
      const orderSource = source || currentOrder?.source || 'admin'
      
      const table = orderSource === 'webshop' ? 'webshop_orders' : 'orders'
  ...
  const sendToProduction = async (order: Order) => {
    try {
      ...
      // Smart logic: check stock and delivery date for webshop orders
      if (order.source === 'webshop') {
        const deliveryDate = order.pickup_date ? new Date(order.pickup_date) : null
        const now = new Date()
        const tomorrow = new Date(now)
        tomorrow.setDate(tomorrow.getHours() >= 14 ? tomorrow.getDate() + 1 : tomorrow.getDate())
        ...
        for (const item of order.items) {
          const { data: inv } = await supabase
            ...
            .maybeSingle()
          if (!inv || (inv.current_stock ?? 0) < (item.quantity || 1)) {
            allInStock = false
            break
          }
        }
        ...
        if (allInStock) {
          // Items are in stock — no production needed, just confirm
          const confirm1 = confirm(
            `✅ Minden termék készleten van!\n\nKészletes teljesítés: nem szükséges gyártás.\n\nAz állapot "feldolgozás alatt"-ra változik.\n\nFolytatod?`
          )
          if (!confirm1) return
          await handleUpdateStatus(order.id, 'processing', order.source)
          toast.success('Rendelés megerősítve — készletből teljesítve!')
          return
        }

        // Items not fully in stock — check if late order (after 14:00) for next day
        const isLateOrder = now.getHours() >= 14
        const isNextDay = deliveryDate && deliveryDate.toDateString() === tomorrow.toDateString()

        if (isLateOrder && isNextDay) {
          const confirm2 = confirm(
            `⚠️ Késői rendelés! (${now.toLocaleTimeString('hu-HU', { hour: '2-digit', minute: '2-digit' })})\n\n` +
            `A rendelés másnap (${deliveryDate?.toLocaleDateString('hu-HU')}) esedékes.\n\n` +
            `Gyártásba küldés MA még? Igen = gyártás indul. Nem = holnap reggel gyártás.`
          )
          if (!confirm2) {
            // Schedule for tomorrow — just update status
            await handleUpdateStatus(order.id, 'confirmed', order.source)
            toast.success('Rendelés visszaigazolva — holnap reggel gyártásba kerül.')
            return
          }
        }
      }
              ...
              {filteredOrders.map((order) => (
                <tr key={order.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                    {new Date(order.order_date || order.created_at).toLocaleDateString('hu-HU')}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    {order.source === 'webshop' && (
