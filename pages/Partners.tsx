...
export default function Partners() {
  ...
  const [showPartnerDetail, setShowPartnerDetail] = useState<Partner | null>(null)
  const [partnerDetailTab, setPartnerDetailTab] = useState<'info' | 'deliveries' | 'invoices'>('info')
  const [partnerDeliveries, setPartnerDeliveries] = useState<any[]>([])
  const [partnerInvoices, setPartnerInvoices] = useState<any[]>([])
  const [partnerDocsLoading, setPartnerDocsLoading] = useState(false)
  ...
  const loadPartnerDocs = async (partner: Partner) => {
    setPartnerDocsLoading(true)
    try {
      const [{ data: dn }, { data: inv }] = await Promise.all([
        supabase.from('delivery_notes')
          .select('*')
          .ilike('customer_name', `%${partner.name}%`)
      ...
      setPartnerDeliveries(dn || [])
      setPartnerInvoices(inv || [])
    } finally {
      setPartnerDocsLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
                  ...
                  <tr key={partner.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex justify-end space-x-2">
                        ...
                        <button
                          onClick={() => { setShowPartnerDetail(partner); setPartnerDetailTab('info'); loadPartnerDocs(partner) }}
                          className="text-purple-600 hover:text-purple-900 dark:text-purple-400 dark:hover:text-purple-300"
                          title="Szállítólevelek & Számlák"
                        >
      ...
      {/* Partner Detail Modal – Szállítólevelek & Számlák */}
      {showPartnerDetail && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4"
          onClick={() => setShowPartnerDetail(null)}>
          <div className="bg-white dark:bg-gray-900 rounded-3xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl"
            onClick={e => e.stopPropagation()}>
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
              <div>
                <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Partner</p>
                <h2 className="text-lg font-black text-gray-900 dark:text-white">{showPartnerDetail.name}</h2>
              </div>
              <button onClick={() => setShowPartnerDetail(null)}
                className="p-2 bg-gray-100 dark:bg-gray-800 rounded-xl text-gray-500">
                <X className="h-5 w-5" />
              </button>
            </div>
            {/* Tabs */}
            <div className="flex gap-1 px-6 py-3 border-b border-gray-200 dark:border-gray-700">
              {([['info', 'Adatok'], ['deliveries', 'Szállítólevelek'], ['invoices', 'Számlák']] as const).map(([tab, label]) => (
                <button key={tab} onClick={() => setPartnerDetailTab(tab)}
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {partnerDetailTab === 'info' && (
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      { label: 'Email', val: showPartnerDetail.email || '—' },
                      { label: 'Telefon', val: showPartnerDetail.phone || '—' },
                      { label: 'Kapcsolattartó', val: showPartnerDetail.contact_person || '—' },
                      { label: 'Adószám', val: showPartnerDetail.tax_number || '—' },
                      { label: 'Város', val: showPartnerDetail.city || '—' },
                      { label: 'Cím', val: showPartnerDetail.address || '—' },
                      { label: 'Kedvezmény', val: `${showPartnerDetail.discount_percentage || 0}%` },
                      { label: 'Fizetési feltétel', val: showPartnerDetail.payment_terms || 'Azonnali' },
                    ].map(item => (
                      <div key={item.label} className="bg-gray-50 dark:bg-gray-800 rounded-xl p-3">
                        <p className="text-xs text-gray-400 uppercase font-bold tracking-widest mb-1">{item.label}</p>
                        <p className="text-sm font-semibold text-gray-900 dark:text-white">{item.val}</p>
                      </div>
                    ))}
                  </div>
                  {/* Payment due days setting */}
                  <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-500/30 rounded-2xl p-4">
                    <p className="text-sm font-black text-amber-700 dark:text-amber-400 mb-3">Számla fizetési határidő beállítása</p>
                    <div className="flex gap-2 flex-wrap">
                      {[['immediate', 'Azonnali (0 nap)'], ['15', '15 nap'], ['30', '30 nap – Automatikus'], ['60', '60 nap'], ['90', '90 nap']].map(([val, label]) => (
                        <button key={val}
                          onClick={async () => {
                            await supabase.from('partner_companies').update({ payment_terms: val }).eq('id', showPartnerDetail.id)
                            setShowPartnerDetail({ ...showPartnerDetail, payment_terms: val })
                            toast.success('Fizetési határidő frissítve!')
                          }}
                          className={`px-3 py-2 rounded-xl text-xs font-bold border transition-all ${
                            showPartnerDetail.payment_terms === val
                ...
                <div>
                  {partnerDocsLoading ? (
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-gray-400">Nincs szállítólevél ehhez a partnerhez</p>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      {partnerDeliveries.map(dn => (
                        <div key={dn.id} className="flex items-center justify-between bg-gray-50 dark:bg-gray-800 rounded-xl px-4 py-3">
                          <div>
                            <p className="font-bold text-sm text-gray-900 dark:text-white">{dn.order_number}</p>
                            <p className="text-xs text-gray-500">{new Date(dn.created_at).toLocaleDateString('hu-HU')}</p>
                          </div>
                          <div className="flex items-center gap-3">
                            <span className={`text-xs font-bold px-2 py-1 rounded-lg ${
                              dn.status === 'delivered' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300' :
                              dn.status === 'in_progress' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300' :
                              'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300'
                            }`}>
                              {dn.status === 'delivered' ? 'Kézbesítve' : dn.status === 'in_progress' ? 'Úton' : 'Függőben'}
                            </span>
                            <p className="text-sm font-black text-gray-900 dark:text-white">
                              {Array.isArray(dn.items) ? dn.items.reduce((s: number, i: any) => s + (i.total_price || 0), 0).toLocaleString('hu-HU') + ' Ft' : '—'}
                            </p>
                          </div>
                        </div>
                      ))}
                ...
                <div>
                  {partnerDocsLoading ? (
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-gray-400">Nincs számla ehhez a partnerhez</p>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      {partnerInvoices.map(inv => {
                        ...
                        return (
                          <div key={inv.id} className={`flex items-center justify-between rounded-xl px-4 py-3 border ${isOverdue ? 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-500/30' : 'bg-gray-50 dark:bg-gray-800 border-transparent'}`}>
                            <div>
                              <p className="font-bold text-sm text-gray-900 dark:text-white">{inv.invoice_number}</p>
                              <p className="text-xs text-gray-500">Kiállítva: {inv.issue_date} | Határidő: {inv.due_date}</p>
                              {isOverdue && <p className="text-xs text-red-600 font-bold">⚠️ Lejárt!</p>}
                            </div>
                            <div className="text-right">
                              <p className="text-sm font-black text-gray-900 dark:text-white">{(inv.total_amount || 0).toLocaleString('hu-HU')} Ft</p>
                              <span className={`text-xs font-bold px-2 py-0.5 rounded-lg ${
                                inv.payment_status === 'paid' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300' :
                                isOverdue ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300' :
                                'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300'
                              }`}>
                                {inv.payment_status === 'paid' ? 'Fizetve' : isOverdue ? 'LEJÁRT' : 'Nyitott'}
                              </span>
                            </div>
                          </div>
                        )
