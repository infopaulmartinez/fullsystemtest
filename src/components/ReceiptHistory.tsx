...
import { Receipt, Download, ChevronDown, ChevronUp, Calendar, Banknote, CreditCard, CheckCircle, XCircle, Printer } from 'lucide-react'
import { openReceipt, type ReceiptData } from '../lib/receiptGenerator'
...
interface ReceiptHistoryItem {
  ...
  cashier_id: string
  payment_method: 'cash' | 'card' | 'cashmatic_cash'
  amount_total: number
  success: boolean
  created_at: string
  receipt_data: any
...
export function ReceiptHistory() {
  ...
  function reopenReceipt(item: ReceiptHistoryItem) {
    if (!item.receipt_data) { return }
    const data: ReceiptData = {
      ...item.receipt_data,
      date: new Date(item.receipt_data.date ?? item.created_at),
    }
    openReceipt(data)
  }
  ...
  async function loadReceipts() {
    ...
    try {
      ...
      if (filter !== 'all') {
        if (filter === 'cash') {
          query = query.in('payment_method', ['cash', 'cashmatic_cash'])
        } else {
            ...
            >
              ...
              >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${
                          receipt.payment_method === 'cash' || receipt.payment_method === 'cashmatic_cash'
                ...
                <div className="flex items-center gap-1">
                  <button
                    onClick={e => { e.stopPropagation(); reopenReceipt(receipt) }}
                    title="Blokk újranyitása"
