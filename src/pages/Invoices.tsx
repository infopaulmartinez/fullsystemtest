    ...
    if (selectedNote) {
      setFormData(prev => ({
        ...
        items: selectedNote.items.map((item: any) => {
          // Ha van total_amount és tax_rate, ebből számítunk unit_price-t
          let unitPrice = item.unit_price || item.price || 0
          const quantity = item.quantity || 1
          const taxRate = item.tax_rate || 27
          const totalAmount = item.total_amount
          
          // Ha nincs unit_price de van total_amount, akkor vissza-számítunk
          if ((!unitPrice || unitPrice === 0) && totalAmount) {
            unitPrice = totalAmount / (quantity * (1 + taxRate / 100))
          }
          
          const taxAmount = totalAmount ? (totalAmount * (taxRate / (100 + taxRate))) : ((quantity * unitPrice) * (taxRate / 100))
          ...
          return {
            ...
            tax_rate: taxRate,
            tax_amount: taxAmount,
            total_amount: totalAmount || (quantity * unitPrice * (1 + taxRate / 100))
          }
        })
      }))
    }
