...
export default function GPSTrackingDashboard({ selectedVehicle, onVehicleSelect }: GPSTrackingDashboardProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(loadData, 180000) // Update every 3 minutes (rate limit friendly)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Public Map Link Button */}
      <div className="flex justify-end mt-4">
        <button
          onClick={() => window.open('https://fleet.trackgps.ro/public-map/9DF91D48-3FA1-484B-AA96-50C261A8122C', '_blank')}
          className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
        >
          <Map className="h-5 w-5 mr-2" />
          Nyilvános Térkép Megnyitása
        </button>
      </div>
